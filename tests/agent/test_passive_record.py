...
@pytest.mark.asyncio
async def test_passive_record_with_image_stores_media_paths(tmp_path: Path) -> None:
    """A group photo lands with its file path on the entry's ``media`` kwarg.

    No eager vision describe is run — the session manager renders the path as
    an ``[image: <path>]`` breadcrumb on replay (see session/manager.py), and
    the bot can read the file on demand via ``read_file`` when the user
    ...
    msg = InboundMessage(
        ...
        sender_id="42",
        chat_id="-5111011186",
        content="[Alice @alice]: check this out",
        media=[str(photo)],
    ...
    users = [
        m for m in loop.sessions.get_or_create("telegram:-5111011186").messages
        if m.get("role") == "user"
    ]
    assert users, "expected the passive photo message to land in history"
    persisted = users[-1]
    # Text persists verbatim — no describe injection.
    assert persisted["content"] == "[Alice @alice]: check this out"
    # The file path rides on ``media`` so history replay + read_file can find it.
    assert persisted.get("media") == [str(photo)]


@pytest.mark.asyncio
async def test_passive_record_media_only_still_persists(tmp_path: Path) -> None:
    """Photo-only group messages (empty text) still land in history with media.

    Before this path, the empty-text early-return would have dropped the
    message entirely; with media attached, the entry survives so the
    breadcrumb shows up on replay.
    """
    loop = _make_loop(tmp_path)
    ...
    assert users, "media-only passive message must still be recorded"
