...
def _run_gateway(
    ...
    *,
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        Upstream's pattern (run heartbeat / dream / agent_turn inline through
        the main agent's `process_direct`) was deliberately not taken: the
        fork relies on subagent isolation + the `cron_result` event path so
        scheduled work can't pollute user chat history and the evaluator gate
        runs in `_handle_cron_result` rather than tied to one specific job
        type.
        """
    ...
    agent.dream.curation_model = dream_cfg.curation_model
    ...
    # Upstream added `dream.enabled` to gate registration — we picked the
    # field up in Stage 1 of the merge.
    ...
    # Upstream's heartbeat system job is intentionally NOT registered: its
    # handler is inlined into upstream's on_cron_job (calls agent.process_direct
    # with a temporary `cron_context` token), which is incompatible with the
    # fork's subagent-isolated cron architecture. Porting heartbeat would mean
    # writing a dedicated `register_system_event("heartbeat", ...)` handler
    # that uses _heartbeat_has_active_tasks() + _pick_heartbeat_target() and
    # respects the subagent boundary; out of scope for this merge.
